...
class keep {
    ...
    function listenersActiveNow() 
    {
        global $mysqli;
        $sql = "SELECT country, COUNT(country) listener_count FROM `listeners_active_now` group by country order by listener_count DESC";
        $result = $mysqli->query($sql);
        $listeners = $result->fetch_all( MYSQLI_ASSOC );
    ...
    function telegramCheckRequests( $post )
    {
        global $mysqli;

        extract( $post );

        $sql = "SELECT COUNT(userid) requests, userid, user
                FROM `telegram` WHERE userid in($userid) AND command IN('request')
                AND TIMESTAMPDIFF(HOUR, stamp, NOW()) < 1";
            
        $result = $mysqli->query($sql);
        $requests = $result->fetch_array( MYSQLI_ASSOC );
    ...
    function telegramAction( $post, $id=NULL,$stamp=NULL )
    {
        global $mysqli; 
        ...
        $stmt = $mysqli->prepare("INSERT INTO telegram ( `request`, `command`,`user`,`userid` ) VALUES (?,?,?,?)" );
        $stmt->bind_param( "ssss" , $request, $command, $user, $userid );
        $stmt->execute();      
    ...
    function top10CountryListeners() 
    {
        global $mysqli;
        $mysqli->report_mode = MYSQLI_REPORT_STRICT;

        $sql = "SELECT * FROM `top-10-countries-listeners`";
        $result = $mysqli->query($sql);
    ...
    function soapUpdatePlays( $post )
    {
        ...
        $sql = "SELECT * FROM recording WHERE title='$title'";
        $result = $mysqli->query($sql);
        ...
        {
            $id = $output['id'];
            $sql = "UPDATE recording SET plays=plays+1 WHERE id = $id";
            $mysqli->query($sql);
            return $output;
 ...
 */
    ...
    function insertRelease( $release, $styles='', $genres='' ){
        try{
            ...
            $sql = "INSERT INTO recording (title, artist_id, release_id, last_played) VALUES ('$recording_title' , $artist_id, $discogs_id, '$timestamp' ) ON DUPLICATE KEY UPDATE title=VALUES(title), artist_id=VALUES(artist_id), release_id=VALUES(release_id)";
    ...
    function nerdly(){
        ...
        $result = $mysqli->query($sql) or die('nerdly() function failure: ' . mysql_error($mysqli));
        $out['known_tracks'] = $result->fetch_array( MYSQLI_ASSOC );
 ...
 */
     ...
     */
     ...
     function browseCovers( $release_id ){
         global $mysqli;
         try{
            $output = array('paths' => null, 'release' => null, 'error' => null );

            $sql  = "SELECT DISTINCT( path ) FROM covers WHERE release_id = $release_id";
            
            if( !$results = $mysqli->query($sql)) throw new exception( $mysqli->error );
...
// Lookup, browse and queries for tessa2.js
     ...
     function browseKeep( $artist, $title ){
         ...
         try{
            ...
            $stmt = $mysqli->prepare( $spl );
            $stmt->bind_param( "ss" , $title, $artist );
            $stmt->execute();      
            $result = $stmt->get_result(); // get the mysqli result
            
            // $payload= $result->fetch_assoc(); // fetch data   
        ...
        // if($results->num_rows > 0 ){
                ...
                while( $datapak = $result->fetch_assoc() ){
